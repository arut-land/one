...
//! through [`Localizer`]; `arut-dev generate` turns the same files into
//! `Localizable.xcstrings`, `strings.xml`, `.resw`, and the TypeScript module
//! `@fluent/bundle` reads, so a native surface localizes through its own
//! platform API and never through this crate.
//!
...
//!
//! [`Message`] itself is generated by `build.rs` into `OUT_DIR`: it is a
//! function of the `.ftl` source, so cargo owns its freshness rather than a
//! checked-in file that a generator and `cargo fmt` both rewrite.
//!
...
/// The `Message` enum, emitted by `build.rs` from `locales/en/*.ftl`.
mod generated {
...
/// walks it, so nothing else has a list of locales.
...
impl Localizer {
    ...
    fn over(locales: impl IntoIterator<Item = LanguageIdentifier>) -> Self {
        ...
        for locale in locales {
            ...
            // that tells `arut-dev generate` to emit an integer placeholder on native
...
mod tests {
    ...
    fn an_unknown_language_falls_back_to_the_default_locale() {
        ...
        assert_eq!(
            localizer.format(&Message::ChatErrorCancelled),
            "This conversation closed before your message could send."
        );
    }
