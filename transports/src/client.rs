...
/// Reads a whole response body, refusing one over the message limit whether or
/// not it declared a length.
async fn bounded_body(response: reqwest::Response) -> Result<Vec<u8>, Status> {
    match Limited::new(reqwest::Body::from(response), MAX_MESSAGE)
        .collect()
        .await
    {
        Ok(body) => Ok(body.to_bytes().to_vec()),
        Err(error) if error.is::<LengthLimitError>() => Err(framing::message_limit()),
        Err(error) => Err(Status::new(Code::Unavailable, error.to_string())),
    }
}
