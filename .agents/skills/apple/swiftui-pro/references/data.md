...
- Isolate UI-owned mutable state to the main actor where appropriate. Observation alone does not establish an actor-isolation policy for every model.
- Use Observation when supported by the deployment targets, with `@State` for ownership and `@Bindable` / environment for access as appropriate.
- Retain `ObservableObject` and its property wrappers for supported older targets and existing integration contracts. A review should not create a second source of truth or force a state architecture migration.
...
- Prefer direct bindings for writable local state. Use `Binding(get:set:)` when writes intentionally dispatch commands or adapt another ownership boundary; do not replace that contract with duplicated state and synchronization through `onChange()`.
