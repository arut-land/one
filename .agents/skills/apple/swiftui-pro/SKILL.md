---
name: swiftui-pro
description: Review SwiftUI changes for correctness, accessibility, API availability, and maintainability. Use for code review; select focused implementation or performance skills for those tasks.
license: MIT
metadata:
  author: Paul Hudson
  version: "1.1"
...
- Read the deployment targets and Swift language settings from the project. Adopt modern APIs with availability checks and fallbacks; do not raise minimum OS versions as a side effect of review.
- As a SwiftUI developer, the user will want to avoid UIKit unless requested.
- Prefer existing platform APIs and dependencies; assess a new dependency against the task and repository policy.
- Group small related types when that improves locality; extract independently useful components instead of enforcing one type per file.
...
**Binding review:** A writable local property may use a direct binding. Retain command-backed `Binding(get:set:)` adapters when writes intentionally dispatch to the shared core; replacing them with synchronized local state can change behavior.
...
3. **Data flow:** Check write ownership before proposing a binding change; a command adapter is not itself a defect.
