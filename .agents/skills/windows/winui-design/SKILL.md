...
- `ThemeShadow` casts a shadow from the caster's `Translation` Z. Microsoft's recommended elevations are `16` for tooltips, `32` for popup/flyout UI, `128` for dialogs — pick by surface type. Check the installed Windows App SDK shadow behavior and supported hosting arrangement before adding explicit receivers; do not copy receiver plumbing by default.
...
- Custom theme resources must resolve in light, dark, and high contrast. Keep a valid `Default` fallback where appropriate and prefer system resources.
...
| Irreversible destructive action without an appropriate safeguard | `ContentDialog` with verb-labelled primary action and `Cancel` secondary; surface item identity (name, count) in the body |
