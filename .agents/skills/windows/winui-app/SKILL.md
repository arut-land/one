---
name: winui-app
description: Implement WinUI 3 application structure, lifecycle, windowing, or a new scaffold using Microsoft samples. For visual design, code review, build diagnosis, and UI testing, select the corresponding focused Windows skill.
---
...
2. If the task is about preparing a machine for WinUI, auditing readiness, or creating a brand new app, start with the bundled setup-and-scaffold flow in this skill before broader design, implementation, or troubleshooting work:
   ...
   - For a machine setup task that requires these components, inspect the bundled configuration first. Run it only within the authorized setup scope, from this skill directory:
