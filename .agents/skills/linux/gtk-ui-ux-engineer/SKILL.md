...
- Use system styling first; customize only for a concrete product requirement
...
- Support system appearance using the actual application stack. Arut uses GTK4/Relm4; libadwaita examples apply only when that dependency is intentionally adopted
- Use theme resources and CSS supported by the pinned GTK version
...
**Check CSS against the selected GTK version.** GTK CSS is not browser CSS; the upstream examples, especially media queries and libadwaita styling, need compatibility review before copying. Prefer widget/layout APIs for responsiveness.

Available syntax depends on version:

- ✅ `var(--variable)` for CSS variables (GTK4)
- Check named colors and CSS custom properties against the selected GTK version; do not assume all GTK3-era syntax is unsupported
