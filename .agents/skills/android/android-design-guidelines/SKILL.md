...
description: Material Design 3 and Android platform guidelines. Use when building Android apps with Jetpack Compose, implementing Material You, navigation, or accessibility. Triggers on tasks involving Android UI, Compose components, dynamic color, or Material Design compliance.
license: MIT
metadata:
...
Arut uses Jetpack Compose for Android UI. Keep manifest and resource XML when needed; do not introduce XML/View layouts as the default. Select state, components, motion, performance, or testing skills only for the concern being changed.
