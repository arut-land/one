...
The compact 32-DIP WinUI TitleBar contains the current conversation title, history toggle, search, and new-conversation actions while retaining Windows snap layouts and system controls. The title uses the flexible content column so long names cannot push actions under the caption buttons. Mica and theme resources follow Windows appearance. History changes to an opaque themed overlay below 800 logical pixels. The manifest declares Per-Monitor V2 DPI awareness to prevent Windows from bitmap-scaling the window.
