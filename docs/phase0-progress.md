...
- Browser invalidation delivery is verified under Node against the packed wasm module: subscriptions on the conversation, composer, and list handles each fire on replace and send, the message round-trips, the draft clears, and IDs come from the host. The built web surface was also opened in a browser and a message sent through the UI; the transcript updated.
