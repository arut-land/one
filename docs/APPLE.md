...
Use Xcode 26.0 or newer to compile the Liquid Glass APIs. That is also the toolchain floor both `Package.swift` manifests declare: `swift-tools-version: 6.2` is the level Xcode 26.0 ships, so the packages build in the Swift 6 language mode and strict concurrency is enforced rather than warned about. Both targets set `.defaultIsolation(MainActor.self)` in their manifest, so every declaration in `ArutBindings` and `ArutSurface` is main-actor isolated by that one line instead of by an annotation on each class and initializer. Xcode 26.6, which carries Swift 6.3, is the current stable release and the one the build below was last run on. The app targets macOS 14 and iOS 17, the floor SwiftUI's Observation needs; `project.yml` declares those once under `options.deploymentTarget` instead of per target. Native Liquid Glass controls and `safeAreaBar` are enabled on macOS/iOS 26; earlier systems use bordered controls and `safeAreaInset`. Swift integration tests use the Swift Testing framework bundled with Xcode, which SwiftPM runs directly at this manifest level. The optional macOS UI-test target requires macOS 14 or later.
