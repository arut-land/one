...
1. [x] **Core dependency checks.** `arut-dev check` holds the layer table and the rule that `bindings/ffi` depends on no crate but BoltFFI. The graph assertions moved to the `check:core-graph` mise task, where `cargo tree` proves the wasm `arut_ffi` graph reaches neither wasm-bindgen nor a Tokio executor.
...
12. [x] **Repository gates.** `mise run check` runs nextest, machete, and cargo-deny. Semver checks, feature-powerset builds, and mutation testing remain future CI work. One `arut-dev` binary owns localization generation and the repository checks, including the per-crate reading of `[lints]` that catches a crate weakening the workspace `unsafe_code` deny; `tools/conformance` stays a test crate.
...
| tokio-util | 0.7.19 | MIT | yes (built) | ADOPT NOW | `sync::CancellationToken` + `child_token()` + `DropGuard` mirror the Node→Workspace→Conversation→Operation tree exactly; no feature flags needed, so no `rt` on wasm. `arut-transport` also takes the `codec` feature for its Connect envelope `Decoder`, declared under `[target.'cfg(not(target_arch = "wasm32"))'.dependencies]` so the feature never reaches the wasm core. |
...
| dynosaur | 0.3.1 | MIT OR Apache-2.0 | yes | REJECT (evaluated 2026-09-14) | The only crate that makes `async fn` traits dyn-compatible, prototyped against `RpcChannel`. `Arc` works through `new_arc`, but the erased future is `Pin<Box<dyn Future>>` with no `Send` bound, and every consumer here needs one (`RpcFuture` is `+ Send + 'static`, `Spawner::spawn` takes a `Send` future). Recovering it means stacking `trait_variant::make(Send)` underneath, which adds two generated type names at every `Arc<dyn RpcChannel>` site, and the `Box::pin` it would delete is one expression inside lines that stay. Revisit when dyn-compatible `async fn` in traits lands in the language. |
...
One correction before anyone starts, recorded in ADR 0021's amendment: BoltFFI's source scanner does not expand macros, so a macro that expands *to* `#[export] impl` makes the handle disappear from the generated bindings with no error, even under `--deny-skipped`. Build this as a generator that reads the declaration and writes the `#[export] impl` blocks to disk for the scanner to read, not as an expansion, unless BoltFFI grows macro expansion first.

## Native UI and platform integration from Rust

Verified on crates.io/docs.rs 2026-09-13, GTK floor re-checked 2026-09-14. Build host has GTK 4.22.5 and `fedora:latest` (Fedora 44, released 2026-04-28) ships gtk4-devel 4.22.5, so the `v4_22` feature gate is safe on both. `v4_24` is not: no shipping Fedora carries 4.24.
...
| gtk4 / gdk4 | 0.11.4 (2026-06-29) | MIT | Linux | ADOPT NOW | Already in `surfaces/linux` at `v4_22`, for `gtk_interface_color_scheme()`, `gtk_interface_contrast()` and `gtk_interface_reduced_motion()`. `v4_24` waits on a Fedora that ships 4.24; it is what would retire the last ashpd read, the accent. |
...
| ashpd | 0.13.13 (2026-07-17) | MIT | Linux | ADOPT NOW (`settings`, accent only) / PHASE 4 (rest) | Portals as typed zbus proxies: `settings` (color scheme, accent, contrast, reduced motion + change streams), `background` (autostart), `notification`, `file_chooser`, `secret`, `global_shortcuts`, `inhibit`, `open_uri`. GTK 4.22 reports every appearance setting but the accent, so the surface now uses `accent_color()` + `receive_accent_color_changed()` and nothing else. Features `gtk4` (gives `From<Color> for gdk4::RGBA`), `glib`, `tokio`. |
...
2. Color scheme: build gtk4 with `v4_20` and read `Settings::default().gtk_interface_color_scheme()` (`InterfaceColorScheme::{Unsupported,Default,Dark,Light}`, non-exhaustive), watching `notify::gtk-interface-color-scheme`. GTK 4.20 (2025-08-29) queries the portal itself, so no ashpd is needed for dark mode on a current GTK; keep `ashpd::desktop::settings::Settings::color_scheme()` + `receive_color_scheme_changed()` as the pre-4.20 fallback behind a cfg. The same argument covers decoration layout: `libgtk-4.so.1` carries `org.gnome.desktop.wm.preferences`, `button-layout` and `gtk-decoration-layout`, so GTK reads that portal key itself and maps it into `GtkSettings:gtk-decoration-layout`. Watch `notify::gtk-decoration-layout` rather than running a second ashpd stream for it.
3. Accent: `gtk-accent-color` only appears, unstable, in GTK 4.24 — so read it from the portal now: `ashpd` `settings::Settings::read(APPEARANCE_NAMESPACE, ACCENT_COLOR_SCHEME_KEY)` → `ashpd::desktop::Color` → `gdk4::RGBA` via ashpd's `gtk4` feature, subscribed through `receive_accent_color_changed()`. Out-of-range RGB means "unset": fall back to the theme's own `theme_selected_bg_color`.
4. Reference named theme colors from CSS; do not resolve them in Rust. GTK 4.22's built-in theme `@define-color`s all eight names the surface needs (`theme_bg_color`, `theme_base_color`, `theme_unfocused_bg_color`, `theme_fg_color`, `theme_unfocused_fg_color`, `theme_selected_bg_color`, `theme_selected_fg_color`, `borders`), so a static `style.css` refers to `@theme_bg_color` directly and GTK re-resolves it when the theme changes. That drops WaterUI's `StyleContext::lookup_color` palette walk, the `@define-color arut_*` alias strings built in Rust, the `#[expect(deprecated)]` it needed, and the `notify::gtk-theme-name` re-read. Note 1 is unchanged: no color literal, ever.
...
7. Also read `gtk_interface_contrast()` (v4_20) for high-contrast and `gtk_interface_reduced_motion()` (v4_22) before animating anything, and hand both to the provider (`set_prefers_contrast`, `set_prefers_reduced_motion`) so `style.css` can answer them with `@media (prefers-contrast: more)` and `@media (prefers-reduced-motion: reduce)` instead of Rust appending an override. ashpd's `ReducedMotion` is only needed below 4.22. A desktop that sets `gtk-enable-animations` off is still asking for reduced motion, so fold that setting in before handing the value over.
...
| @fluent/bundle (npm) | 0.19.1 | Apache-2.0 | n/a (browser) | ADOPT NOW | The same Fluent runtime for web, Chromium and the VS Code extension host. It parses the generated `bindings/typescript/src/generated/catalog.ts` module, which carries each locale's `.ftl` text, so no surface fetches string files beside its own bundle. With `@fluent/langneg` 0.7.0 (Apache-2.0) negotiating `navigator.languages` against that module's `locales`. |
...
- `mise run check` uses nextest and cargo-machete in `check:rust` and runs cargo-deny as its own task, `check:deps`, which may update its advisory cache over the network and is a separate CI job for that reason.
...
- ADR 0022 string source: `product/i18n` embeds Fluent for Rust localization; `arut-dev generate` reads that source through the shared `arut-i18n-catalog` parser to write resources and typed accessors. GTK uses Rust Fluent at runtime. Apple, Android, and Windows use native resources; TypeScript uses `@fluent/bundle`. `fluent-fallback` and `fluent-resmgr` are not needed for this build-time resource pipeline.
